...
const UserHomeNotificationItem = ({
  notification,
  type,
  index,
  isBlur = false,
  ...
  return (
    <NotificationLayout
      isBlur={isBlur}
      onClick={() =>
        navigate(`/dashboard/${path}`, { state: { type: type, index: index } })
      }
      image={userImage}
  ...
  function getUser(notication) {
    return notication.participants[0].id !== auth.id
      ? notification?.participants?.[0]
      : notification?.participants?.[1];
  }
  ...
  function getMessageContent(notification, type) {
              ...
              <span
                style={
                  isBlur
                    ? {
                        color: "transparent",
                        textShadow: "0 0 6.4px rgba(230, 73, 151, 1)",
                        marginRight: ".15rem",
                      }
                    : null
                }
    ...
    if (!notification?.lastMessage && type === "message")
      return `İlk adımı at, ${user?.name} seni bekliyor! ✨`;
    else {
      const message =
        notification.lastMessage?.content.length < 30
          ? notification.lastMessage?.content
          : notification.lastMessage?.content.slice(0, 30) + "...";
      return t(
        `NOTIFICATION.QUICK_NOTIFICATIONS.${notification?.lastMessage?.type.toUpperCase()}`,
