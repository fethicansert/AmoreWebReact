...
const SwipeSlide = ({ galeryPhotos, closeGalery, galeryPhotoIndex }) => {
  const properties = {
    prevArrow: (
      <button className="swipe-slide-arrow-left-button">
        <ArrorHeadLeft
          strokeWidth="2.3"
          width="26"
          height="26"
    ...
    nextArrow: (
      <button className="swipe-slide-arrow-right-button">
        <ArrowHeadRight
          strokeWidth="2.3"
          width="26"
          height="26"
    ...
    >
      <div className="swipe-slide-container">
        <Zoom
          arrows={true}
          {...properties}
          infinite={galeryPhotos.length > 1}
          ...
          easing="ease"
        >
          {galeryPhotos?.map((userPhoto) => (
            <div key={uuidv4()} className="swipe-item-slide-image-box">
              <img
                draggable={false}
                className="swipe-item-slide-image"
                src={userPhoto?.url}
              />
            </div>
          ))}
