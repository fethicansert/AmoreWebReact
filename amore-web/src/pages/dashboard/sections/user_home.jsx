...
const UserHome = () => {
  ...
  useEffect(() => {
    if (
      currentIndex >= Math.floor(swipeList.length / 2) &&
      swipeList.length > 0 &&
  ...
  return (
    <section className="user-home">
      ...
      {!hidePremium && (
        <div className="user-home-sidebar-container">
          <div className="user-home-filters">
            <CurrentUserInfoBox
              style={{ borderBottom: `1px solid ${colors.borderColor1}` }}
            />

            <div
              className={`user-home-filters-options-wrapper ${
                showFilter ? "active" : ""
              }`}
            >
              <FilterSlider
              ...
              <FilterSlider
                ...
                onChangeCommitted={() =>
                  handleFilter({ isAutoDistance: false })
                }
              />
              ...
              >
                ...
                <CustomRadio
                  ...
                  value={"female"}
                  setValue={setFilterGender}
                  isSelected={filterdGender === "female"}
                  onClick={() => handleGender("female")}
                />
              </RadioWrapper>

              <RadioWrapper
                title={t("FILTER.USER_STATUS")}
                style={{ margin: "1rem 0" }}
              >
                <CustomRadio
                  text={t("STATUS.ONLINE")}
                  value={"online"}
                  onClick={() => setFilterUserStatus("online")}
                  isSelected={filterUserStatus === "online"}
                />

                <CustomRadio
                  text={t("STATUS.OFFLINE")}
                  value={"offline"}
                  onClick={() => setFilterUserStatus("offline")}
                  isSelected={filterUserStatus === "offline"}
                />
              </RadioWrapper>
            </div>
            ...
            >
              <BasicButton
                fontSize={showFilter ? ".75rem" : ".8rem"}
      ...
      <div
        ...
        ref={swipeContainer}
        style={{ borderRadius: showLocationBanner ? "0" : "0 0 12px 12px" }}
      >
        {swipeError ? (
          <SwipeErrorContainer
            errorText={t("ERRORS.UNEXPECTED_ERROR.TITLE")}
            subErrorText={t("ERRORS.UNEXPECTED_ERROR.SUB_TITLE")}
            animation={somethingWentWrongLottie}
            width={"62%"}
          />
        ) : !isSwipeListLoading &&
          swipeList.length === 0 &&
          ...
          <SwipeErrorContainer
            errorText={t("DASHBOARD.TITLES.FILTER_CHANGE_TITLE")}
            subErrorText={t("DASHBOARD.TITLES.FILTER_CHANGE_SUB_TITLE")}
            animation={locationVaveLottie}
            width={"70%"}
          />
        ) : (
          <>
            ...
            {
              <SwipeItem
                user={swipeList[currentIndex]}
                loading={isSwipeListLoading || swipeList.length === 0}
                isUserBlocked={false}
              />
            }

            {!isSwipeListLoading && swipeList.length !== 0 && (
              <SwipeBottomBar onMessage={null} onSwipe={handleSwipe} />
            )}

            {popAnimation && (
              <div
                className="like-popup"
                style={{ marginTop: `${swipeContainer?.current?.scrollTop}px` }}
              >
                {popAnimation.icon}
              </div>
            )}
            ...
            {showUserMatchesPopup && (
              <UsersMatchPopup
                ...
                images={[
                  auth?.photos?.[0]?.url,
                  matchedUserRef.current?.photos?.[0].url,
                ]}
                matchedName={matchedUserRef.current?.name}
              />
            )}
          </>
        )}
  ...
  async function getSwipeList({
    ...
    try {
      const response = await axiosAmore.get(
        `user/discover?minAge=${age[0]}&maxAge=${
          age[1]
        }&isOnline=true&distance=${
          isAutoDistance ? distanceRef.current : distance
        }&gender=${gender}&ids=${userViewedIds.current.join(",")}`,
        {
      ...
      if (response.data.response.code === 200) {
        ...
        if (fetchedList.length > 0)
          fetchedList.forEach((user) => userViewedIds.current.push(user.id));

        //If isResetList SET fetched data to swipeList if not ADD fetched data to swipeList
        setSwipeList((prev) =>
          isResetList ? [...fetchedList] : [...prev, ...fetchedList]
        );
  ...
  async function updateDistance({ gender, age }) {
    ...
    try {
      const response = await axiosAmore.get(
        `user/discover_count?minAge=${age[0]}&maxAge=${
          age[1]
        }&isOnline=true&distance=${
          distanceRef.current
        }&gender=${gender}&ids=${userViewedIds.current.join(",")}`,
