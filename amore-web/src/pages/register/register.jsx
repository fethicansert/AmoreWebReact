...
const Register = () => {
  ...
  //CONSTANTS
  const titles = registerLocalization.map((localization) =>
    t(`REGISTER.${localization}.TITLE`)
  );
  const infos = registerLocalization.map((localization) =>
    t(`REGISTER.${localization}.INFO`)
  );
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        if (
          e.target.parentElement.className === "register-birth-date" ||
          ...
          <div className="register-wrapper">
            <div className="register-container">
              ...
              {infos[currentPageIndex] && (
                <p className="register-info-text">{infos[currentPageIndex]}</p>
              )}
              ...
              <BasicButton
                ...
                onClick={() =>
                  currentPageIndex === 0
                    ...
                    ? verifyOtp()
                    : currentPageIndex !== 7
                    ? navigateForward({ checkError: true })
                    : register()
                }
                width={"100%"}
  ...
  function getCurrentSection() {
    switch (currentPageIndex) {
      case 0:
        return <OtpRegister phone={phone} setPhone={setPhone} />;
      case 1:
        return (
          <VerifyOtp phone={phone} smsCode={smsCode} setSmsCode={setSmsCode} />
        );
      case 2:
      ...
      case 4:
        return <RegisterGender gender={gender} setGender={setGender} />;
      case 5:
        return (
          <RegisterHobbies
  ...
  async function getOtp() {
    ...
    if (request?.status === 200) {
      ...
      if (request.errorMessage === "PHONE_NUMBER_INVALID")
        ...
        setError(
          t(`REGISTER.PHONE.WAIT_NEW_CODE_ERROR`, {
            otpCooldown: request?.cooldown,
          })
        );
  ...
  function handleDeleteImage(imageIndex) {
    setUserImages((prev) =>
      prev.filter((img, index) => (imageIndex === index ? undefined : img))
    );
  }
  ...
  function validateInputs() {
    switch (currentPageIndex) {
      ...
      case 6:
        return validateUserImages();
      case 7:
        return validatUserLocation();
      default:
        return false;
    }
  }
