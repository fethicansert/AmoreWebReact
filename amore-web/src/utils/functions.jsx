...
//Create OTP Request
export const createOtp = async ({ phone, onStart, onFinally }) => {
  if (onStart) onStart();
  const body = {
    phone,
  };
...
//User login request
export const login = async ({ phone, otpId, otpCode, onStart, onFinally }) => {
  if (onStart) onStart();
  const body = {
    phone,
    otpId,
...
export function handleLocationPermission({
  onGranted,
  onDenied,
  onPositionReveal,
  onPositionDenied,
  ...
  navigator.permissions.query({ name: permissionType }).then((result) => {
    ...
    } else if (result.state === "denied") onDenied?.();
...
//
export function handlePushPermission({
  onGranted,
  onDenied,
  onPromptGranted,
  onPromptDenied,
...
export function checkScrollThresold({
  e,
  card,
  rowLength,
  columnCount,
  ...
  const threshold =
    ((cardHeight * rowLength) / columnCount) * thresholdPercentage;
