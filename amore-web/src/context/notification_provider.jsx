...
const NotificationProvider = ({ children }) => {
  ...
  async function readAllNotifications() {
    try {
      ...
      const response = await axiosAmore.post('notification/read_all', {}, { useAuth: true });
  ...
  async function getUnReadedCount({ showLoading = true }) {
    ...
    try {
      const response = await axiosAmore.get("notification/count", { useAuth: true });
  ...
  async function getNotificationList() {
    ...
    try {
      const response = await axiosAmore.get(
        `notification/get?page=${notficationPage.current}`, { useAuth: true }
      );
